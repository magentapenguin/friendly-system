...
export class Snake {
    ...
    draw(snakeBody: Snake[]) {
        ...
        const sizeModifier = (1-index / snakeBody.length) * 0.7 + 0.3; // Adjust the range as needed
        ...
        if (AI_ENABLED) {
            fillColor = this.collided
                ...
                : `rgb(0, 0, ${255 - Math.min(index / snakeBody.length, 0.8)*255})`;
        } else {
            fillColor = this.collided
                ? `rgb(${255 - colorOffset}, ${colorOffset}, 0)`
                : `rgb(0, ${255 - Math.min(index / snakeBody.length, 0.8)*255}, 0)`;
        }
