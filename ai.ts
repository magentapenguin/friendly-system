...
const checkPastMoves = 5; // Number of previous moves to check for patterns
...
function getRandomDirectionWithWeight(exclude: Direction, weightedDirection: Direction, weight: number = 0.5): Direction {
...
export function getBestMove(snake: Snake[], apple: Apple): Direction {
    ...
    const filteredMoves = possibleMoves.filter(move => move !== oppositeDirections[head.direction]);
    ...
    const movesToEvaluate = filteredMoves.length > 0 ? filteredMoves : possibleMoves;
    ...
    for (const initialMove of movesToEvaluate) {
        ...
        for (let step = 0; step < lookAhead; step++) {
            ...
            const distance = Math.abs(currentHead.x - apple.x) + Math.abs(currentHead.y - apple.y);
            ...
            for (const nextMove of directions) {
                ...
                const nextDistance = Math.abs(nextHead.x - apple.x) + Math.abs(nextHead.y - apple.y);
    ...
    if (bestMove === null) {
        ...
        console.groupEnd();
        return getRandomDirectionWithWeight(head.direction, oppositeDirections[head.direction]);
    }
    console.groupEnd();
    return bestMove;
}
...
export function getNextMove(snake: Snake[], apple: Apple): Direction {
    ...
    while (isCollision(snake, nextDirection)) {
        ...
        nextDirection = getRandomDirectionWithWeight(snake[0].direction, oppositeDirections[snake[0].direction]);
        console.log('AI: Collision detected, trying new direction:', nextDirection);
    }
