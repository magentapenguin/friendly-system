...
function drawUI() {
    ...
    if (!gameOver) {
        ...
        ctx.fillText('Controls: Arrow Keys / WASD / Swipe / Gamepad', canvas.width - 10, 10);
    }
    ...
    if (gameOver) {
        ...
        if (Math.sin(time * 5) > 0) {
            ctx.font = '18px Arial';
            ctx.fillText('Press SPACE, Touch, or Gamepad A to restart', canvas.width / 2, canvas.height / 2 + 95);
        }
    }
}
...
function draw(timestamp = 0) {
    ...
    if (snakeBody.length > 0 && !gameOver) {
        ...
        ctx.beginPath();
        ctx.arc(head.visualX * size + size/2, head.visualY * size + size/2, 
                size * 0.7 + Math.sin(timestamp * 0.005) * 3, 0, Math.PI * 2);
...
function applyBloom() {
    ...
    bloomCtx.filter = `brightness(${1 + visualEffects.bloom.strength}) contrast(${1 + visualEffects.bloom.threshold})`;
...
function applyCrtEffect() {
    ...
    const { scanlineIntensity, flickerIntensity, noiseIntensity, curvature } = visualEffects.crt;
